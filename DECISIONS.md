...
## 41. 2026-09-25 · Дизайн-токены: внешний вид только из `src/styles.css`

Готовимся к смене UX и дизайн-системы по референсу - смена стиля должна быть правкой переменных, а не переписыванием компонентов. Базой взяты токены shadcn (CSS-переменные в `:root`/`.dark` → `@theme inline`), а не свой слой поверх: shadcn-компоненты уже на них, второй слой дал бы две системы имён. Правило: наши компоненты и роуты используют только семантические классы (цвета/радиусы shadcn, стандартная шкала Tailwind для отступов и типографики), без палитры, hex и произвольных значений. Аудит сырых цветов не нашёл; реальная несогласованность была в раскладке - контейнер страницы и заголовок копировались по 5 роутам и `MessageScreen`. Вынесены в `PageShell` (варианты `page`/`auth`/`message`) и `PageTitle`, ширины - токены `--container-page`/`--container-auth` (пиксели те же: 28rem/24rem). Отвергнуто: отдельные токены отступов (`--spacing-page` и т.п.) - шкала Tailwind уже единая, лишний слой без пользы; линтер на сырые цвета - вне скоупа (issue #12).
