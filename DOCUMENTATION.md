...
- `src/components/page-shell.tsx` - `PageShell`: фон страницы + центрированный контейнер; варианты `page` (список/формы), `auth` (логин/регистрация), `message` (центрированный экран).
- `src/components/page-title.tsx` - `PageTitle`: заголовок экрана (`h1`).
- `src/components/message-screen.tsx` - общий центрированный экран на `PageShell variant="message"` (`icon/title/description/action`) для 404 / notFound / error.
...
- **Дизайн-токены:** единственный источник внешнего вида - `src/styles.css`. Цвета и радиус - CSS-переменные shadcn в `:root` (и `.dark`), в Tailwind попадают через `@theme inline` (`bg-background`, `text-muted-foreground`, `border-border`, `rounded-lg` и т.д.). Ширины контейнеров - `@theme` (`--container-page` 28rem → `max-w-page`, `--container-auth` 24rem → `max-w-auth`). Правило: в наших компонентах и роутах только семантические классы - без палитры Tailwind (`gray-500`, `white`), hex и произвольных значений (`[13px]`); модификатор прозрачности токена допустим (`bg-muted/30`). Стандартная шкала Tailwind для отступов/размеров/типографики (`p-4`, `gap-2`, `text-sm`) - это тоже токены. Новый токен: цвет - переменная в `:root` и `.dark` + `--color-<name>: var(--<name>)` в `@theme inline`; не цвет - в `@theme` в нужном namespace (`--container-*`, `--radius-*`, `--shadow-*`). Повторяющуюся раскладку выносим в компонент (`PageShell`, `PageTitle`), а не копируем классы. `src/components/ui/*` (shadcn) не правим руками.
