...
- **Тема:** `next-themes` `ThemeProvider` (`attribute="class"`, `defaultTheme="system"`) в `__root.tsx` вокруг `children`/`Toaster`/Analytics. Переключатель `ThemeToggle` в `todo-header.tsx`, цикл light → dark → system. Выбор хранится в `localStorage` (ключ `theme`).
